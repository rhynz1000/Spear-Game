	...
	while (!file.eof()) {
		...
		for (char item : line) {
			...
			if (item == '1') {
				...
				temp->Initalise(camera, size, size, -(x * size + size / 2.0f - SCR_WIDTH / 2.0f), -(y * size + size / 2.0f - SCR_HEIGHT / 2.0f), prog, underground ? TextureLoader::get("ground_mid") : TextureLoader::get("ground"));
