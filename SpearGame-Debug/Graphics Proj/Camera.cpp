...
void CCamera::orthoInti(float scrWidth, float scrHeight, float near, float far)//initalise an ortographic camera
{
	float halfSCRWidth = scrWidth * 0.5f;
	float halfSCRHeight = scrHeight * 0.5f;
	proj = glm::ortho(-halfSCRWidth, halfSCRWidth, -halfSCRHeight, halfSCRHeight, near, far);
}

void CCamera::perspInti(float scrWidth, float scrHeight, float fov, float near, float far)//initalise a perspective camera
{
	proj = glm::perspective(fov, scrWidth/scrHeight, near, far);
}
